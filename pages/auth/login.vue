<template>
    <div
        class="relative md:h-screen sm:py-16 py-36 flex items-center bg-gradient-to-b from-primary/5 via-primary/5 to-primary/10">
        <div class="container">
            <div class="flex justify-center items-center lg:max-w-lg">
                <div class="flex flex-col h-full">
                    <div class="shrink">
                        <div>
                            <a href="home.html" class="flex items-center">
                                <img src="/assets/logo-dark-6dbab3e1.png" alt="logo" class="h-12 flex dark:hidden">
                                <img src="/assets/logo-light-35c89c2c.png" alt="logo" class="h-12 hidden dark:flex">
                            </a>
                        </div>
                        <div class="py-10">
                            <h1 class="text-3xl font-semibold text-default-800 mb-2">Login</h1>
                            <p class="text-sm text-default-500 max-w-md">Lorem ipsum dolor sit amet, consectetur adipiscing
                                elit, sed do eiusmod taempor.</p>
                        </div>
                        <form data-x-form="" data-x-form-to="home.html">
                            <div class="mb-6">
                                <label class="block text-sm font-medium text-default-900 mb-2"
                                    for="LoggingEmailAddress">Email</label>
                                <input data-x-field="email" id="LoggingEmailAddress" value="user@demo.com"
                                    class="block w-full rounded-full py-2.5 px-4 bg-white border border-default-200 focus:ring-transparent focus:border-default-200 dark:bg-default-50"
                                    type="email" placeholder="Enter your email" fdprocessedid="zjvux">
                                <span data-x-field-error="email" class="text-red-500"></span>
                            </div>
                            <div class="mb-6">
                                <div class="flex items-center justify-between mb-2">
                                    <label class="block text-sm font-medium text-default-900"
                                        for="form-password">Password</label>
                                    <a href="auth-recoverpw.html" class="text-xs text-default-700">Forget Password ?</a>
                                </div>
                                <div class="flex" data-x-password="">
                                    <input data-x-field="password" value="password" type="password" id="form-password"
                                        class="form-password block w-full rounded-s-full py-2.5 px-4 bg-white border border-default-200 focus:ring-transparent focus:border-default-200 dark:bg-default-50"
                                        placeholder="Enter your password" fdprocessedid="hixspl">
                                    <button type="button" id="password-addon"
                                        class="password-toggle inline-flex items-center justify-center py-2.5 px-4 border rounded-e-full bg-white -ms-px border-default-200 dark:bg-default-50"
                                        fdprocessedid="i00ftd">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                            stroke-linejoin="round" data-lucide="eye"
                                            class="lucide lucide-eye password-eye-on h-5 w-5 text-default-600">
                                            <path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"></path>
                                            <circle cx="12" cy="12" r="3"></circle>
                                        </svg>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                            stroke-linejoin="round" data-lucide="eye-off"
                                            class="lucide lucide-eye-off password-eye-off h-5 w-5 text-default-600 hidden">
                                            <path d="M9.88 9.88a3 3 0 1 0 4.24 4.24"></path>
                                            <path
                                                d="M10.73 5.08A10.43 10.43 0 0 1 12 5c7 0 10 7 10 7a13.16 13.16 0 0 1-1.67 2.68">
                                            </path>
                                            <path
                                                d="M6.61 6.61A13.526 13.526 0 0 0 2 12s3 7 10 7a9.74 9.74 0 0 0 5.39-1.61">
                                            </path>
                                            <line x1="2" x2="22" y1="2" y2="22"></line>
                                        </svg>
                                    </button>
                                </div>
                                <span data-x-field-error="password" class="text-red-500"></span>
                            </div>
                            <div class="flex justify-center mb-6">
                                <button type="submit"
                                    class="relative inline-flex items-center justify-center px-6 py-3 rounded-full text-base bg-primary text-white capitalize transition-all hover:bg-primary-500 w-full"
                                    fdprocessedid="k0a0l">Log In </button>
                            </div>
                            <div class="mb-6">
                                <div class="flex items-center justify-center gap-4">
                                    <a href="javascript:void(0)" class="">
                                        <img src="/assets/google-48c5b900.svg" class="h-8 w-8">
                                    </a>
                                    <a href="javascript:void(0)" class="">
                                        <img src="/assets/facebook-b2928e06.svg" class="h-8 w-8">
                                    </a>
                                </div>
                            </div>
                        </form>
                    </div>

                    <div class="grow flex items-end justify-center mt-16">
                        <p class="text-default-950 text-center mt-auto">Don’t have an account ? <a href="auth-register.html"
                                class="text-primary ms-1"><span class="font-medium">Register</span></a></p>
                    </div>
                </div>
            </div>
        </div>

        <div>
            <div class="absolute top-1/2 -translate-y-1/3 start-0 end-0 w-full -z-10">
                <img src="data:image/png;base64,"
                    class="w-full opacity-50 flex">
            </div>

            <div class="absolute top-0 end-0 hidden xl:flex h-5/6">
                <img src="/assets/auth-bg-d79436e2.png" class="w-full z-0">
            </div>
        </div>
    </div>
</template>